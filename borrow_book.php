    ...
    if ($emprunt->isBookAvailable($bookId)) {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (strtotime($dateRetourPrevue) > strtotime(date('Y-m-d')) && strtotime($dateRetourPrevue) <= strtotime('+30 days')) {
                ...
                $message = "La date de retour doit être comprise entre aujourd'hui et 30 jours à partir d'aujourd'hui.";
            }
        } else {
        ...
        $message = "Le livre n'est pas disponible pour emprunt.";
    }
} else {
